	...
	*****************************************************************************/
	...
	static unsafe int Main()
	{
			...
			CertStoreFlags.CERT_SYSTEM_STORE_CURRENT_USER, "MY"))
		{
			...
			using (var prgpSelection = CertSelectCertificateChains(default, CertSelection.CERT_SELECT_TRUSTED_ROOT | CertSelection.CERT_SELECT_HAS_PRIVATE_KEY,
				default, rgCriteriaFilter, hStore))
			{
				if ((IntPtr)prgpSelection == default)
				{
					hr = (HRESULT)Win32Error.GetLastError();
					goto CleanUp;
				}

				if (prgpSelection.Count < 1)
				{
				...
				var certInput = new CERT_SELECTUI_INPUT
				{
					...
					cChain = (uint)prgpSelection.Count
				};
