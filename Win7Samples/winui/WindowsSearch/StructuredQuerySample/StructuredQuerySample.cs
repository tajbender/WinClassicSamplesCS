...
// It is possible to CoCreateInstance a query parser directly using __uuidof(QueryParser) but by using a QueryParserManager we can get a
// query parser prepared for a certain UI language and catalog.
var pqpm = new IQueryParserManager();

// The language given to IQueryParserManager::CreateLoadedParser should be the user interface language of the application, which is often the
// same as the operating system's default UI language for this user. It is used for deciding what language to be used when interpreting keywords.
var pqp = pqpm.CreateLoadedParser<IQueryParser>("SystemIndex", GetUserDefaultUILanguage());

// This sample turns off "natural query syntax", which is the more relaxed and human language like search notation, and also makes searches
// automatically look for words beginning with what has been specified (so an input of "from:bill" will search for anything from someone
// whose name begins with "bill".
pqpm.InitializeOptions(false, true, pqp);

string? szLine;
while (!string.IsNullOrEmpty(szLine = Console.ReadLine()))
{
	// The actual work of parsing a query string is done here.
	var pqs = pqp.Parse(szLine.ToString());

	// In this sample we do not bother distinguishing between various parse errors though we could. Note that there will be a solution even
	// if there were parse errors; it just may not be what the user intended.
	...
	// IQueryCondition::Resolve and IConditionFactory2::ResolveCondition turn any date/time references (relative, such as "today", and
	// absolute, such as "5/7/2009") into absolute date/time references (in the UTC time zone), and also simplifies the result in various
	// ways. Note that pc is unchanged and could be resolved against additional dates/times. Code that targets only Windows 7 and later can
	// take advantage of IConditionFactory2::ResolveCondition. Code that targets also earlier versions of Windows and Windows Search should
	// use IQueryCondition::Resolve.
...
static void DisplayQuery(ICondition2 pc, int cIndentation)
{
		...
		case CONDITION_TYPE.CT_LEAF_CONDITION:
			{
				...
				pc.GetLeafConditionInfo(out var propkey, out var op, out propvar.GetRefValue());
				var hr = PSGetPropertyDescription(propkey, out IPropertyDescription? ppd);
				if (hr.Succeeded)
				{
					string pszPropertyName = ppd!.GetCanonicalName();
					hr = PropVariantChangeType(out PROPVARIANT propvarString, propvar, PROPVAR_CHANGE_FLAGS.PVCHF_ALPHABOOL, VARTYPE.VT_LPWSTR); // Real applications should prefer PSFormatForDisplay but we want more "raw" values.
					if (hr.Succeeded)
					{
...
static string GetOperationLabel(CONDITION_OPERATION op) => op.IsValid() ? op.ToString() : "???";