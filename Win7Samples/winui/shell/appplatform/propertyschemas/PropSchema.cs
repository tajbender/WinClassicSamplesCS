...
bool StrCmpICW(string a, string b) => string.Compare(a, b, StringComparison.OrdinalIgnoreCase) != 0;
...
void DumpPropertyDescription(string pszPropertyName)
{
	HRESULT hr = PSGetPropertyDescriptionByName(pszPropertyName, out IPropertyDescription? ppdDump);
	if (hr.Succeeded && ppdDump is not null)
	{
		Console.Write($"{pszPropertyName}\n{new string('-', 30)}\n");

		DumpPropertyKey();
		DumpCanonicalName();
		DumpDisplayName();
		DumpEditInvitation();
		DumpDefaultColumnWidth();
		DumpSortDescriptionLabel();
...
void RegisterSchema(string pszFileName)
{
...
void UnregisterSchema(string pszFileName)
{
...
void DisplayDumpInformation(string[] pszArgList)
{
...
void ParsePropFlag(string pszFlagToUse)
{
