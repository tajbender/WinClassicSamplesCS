...
/// <summary>The FEATURE_NUMBER enumeration provides a list of the features that are defined by the SCSI Multimedia - 4 (MMC-4) specification.</summary>
...
public enum FEATURE_NUMBER : ushort
{
	...
	/// <summary>
	/// Indicates the feature named "Core" by the MMC-3 specification. This feature encompasses the basic functionality which is mandatory
	/// for all devices that support the MMC-3 standard. See the MMC-3 specification for a description of the capabilities included in the
	/// Core feature.
	/// </summary>
	FeatureCore = 0x0001,

	/// <summary>
	/// Indicates the feature named "Morphing" by the MMC-3 specification. Devices that support this feature can notify the initiator of
	/// operational changes and allow the initiator to prevent operational changes.
	/// </summary>
	FeatureMorphing = 0x0002,

	/// <summary>
	/// Indicates the feature named "Removable Medium" by the MMC-3 specification. Devices that support this feature allow the medium to be
	/// removed from the device. They also can communicate to the initiator that the user wants to eject the medium or has inserted a new medium.
	/// </summary>
	FeatureRemovableMedium = 0x0003,

	/// <summary>
	/// Indicates the feature named "Write Protect" by the MMC-3 specification. Devices that support this feature allow the initiator to
	/// change the write-protection state of the media programmatically.
	/// </summary>
	FeatureWriteProtect = 0x0004,

	/// <summary>
	/// Indicates the feature named "Random Readable" by the MMC-3 specification. Devices that support this feature allow the initiator to
	/// read blocks of data on the disk at random locations. These devices do not require that the initiator address disk locations in any
	/// particular order.
	/// </summary>
	FeatureRandomReadable = 0x0010,

	/// <summary>
	/// Indicates the feature named "MultiRead," originally defined by the Optical Storage Technology Association (OSTA) and incorporated
	/// into the MMC-3 specification. Devices that support this feature can read all CD media types.
	/// </summary>
	FeatureMultiRead = 0x001D,

	/// <summary>
	/// Indicates the feature named "CD Read" by the MMC-3 specification. Devices that support this feature can read CD-specific information
	/// from the media and can read user data from all types of CD blocks.
	/// </summary>
	FeatureCdRead = 0x001E,

	/// <summary>
	/// Indicates the feature named "DVD Read" by the MMC-3 specification. Devices that support this feature can read DVD-specific
	/// information from the media.
	/// </summary>
	FeatureDvdRead = 0x001F,

	/// <summary>
	/// Indicates the feature named "Random Writable" by the MMC-3 specification. Devices that support this feature can write blocks of data
	/// to random locations on the disk. These devices do not require that the initiator address disk locations in any particular order.
	/// </summary>
	FeatureRandomWritable = 0x0020,

	/// <summary>
	/// Indicates the feature named "Incremental Streaming Writable" by the MMC-3 specification. Devices that support this feature can append
	/// data to a limited number of locations on the media.
	/// </summary>
	FeatureIncrementalStreamingWritable = 0x0021,

	/// <summary>
	/// Indicates the feature named "Sector Erasable" by the MMC-3 specification. Devices that support this feature require an erase pass
	...
	/// <summary>
	/// Indicates the feature named "Defect Management" by the MMC-3 specification. Devices that support this feature are able to provide
	/// contiguous address space that is guaranteed to be defect free.
	/// </summary>
	FeatureDefectManagement = 0x0024,

	/// <summary>
	/// Indicates the feature named "Write Once" by the MMC-3 specification. Devices that support this feature can write to any previously
	/// unused logical block.
	/// </summary>
	FeatureWriteOnce = 0x0025,

	/// <summary>
	/// Indicates the feature named "Restricted Overwrite" by the MMC-3 specification. Devices that support this feature are limited in
	/// regard to which logical blocks they can overwrite at any given time.
	/// </summary>
	FeatureRestrictedOverwrite = 0x0026,

	/// <summary>
	/// Indicates the feature named "CD-RW CAV Write" by the MMC-3 specification. Devices that support this feature can perform writes on
	/// CD-RW media in CAV mode.
	/// </summary>
	FeatureCdrwCAVWrite = 0x0027,

	/// <summary>
	/// Indicates the feature named "MRW" by the MMC-3 specification. Devices that support this feature can recognize, read and optionally
	/// write MRW formatted media.
	/// </summary>
	FeatureMrw = 0x0028,
	...
	/// <summary>
	/// Indicates the feature named "DVD+RW" by the MMC-3 specification. Devices that support this feature can recognize, read and optionally
	/// write DVD+RW media.
	/// </summary>
	...
	/// <summary>
	/// Indicates the feature named "DVD-RW Restricted Overwrite" by the MMC-3 specification. Devices that support this feature can only
	/// write on block boundaries. These devices cannot perform read or write operations that transfer less than a block of data.
	/// </summary>
	...
	/// <summary>
	/// Indicates the feature named "CD Mastering" by the MMC-3 specification. Devices that support this feature can write to a CD in either
	/// "Session-at-Once" mode or raw mode.
	/// </summary>
	FeatureCdMastering = 0x002E,

	/// <summary>
	/// Indicates the feature named "DVD-R Write" by the MMC-3 specification. Devices that support this feature can write data to a
	/// write-once DVD media in "Disc-at-Once" mode.
	/// </summary>
	...
	/// <summary>
	/// Indicates the feature named "Power Management" by the MMC-3 specification. Devices that support this feature can perform both
	/// initiator and logical-unit directed power management.
	/// </summary>
	FeaturePowerManagement = 0x0100,

	/// <summary>
	/// Indicates the feature named "S.M.A.R.T." by the MMC-3 specification. Devices that support this feature support Self-Monitoring
	/// Analysis and Reporting Technology (SMART).
	/// </summary>
	FeatureSMART = 0x0101,

	/// <summary>
	/// Indicates the feature named "Embedded Changer" by the MMC-3 specification. Devices that support this feature can move media back and
	/// forth between a media storage area and the mechanism that actually accesses the media.
	/// </summary>
	FeatureEmbeddedChanger = 0x0102,

	/// <summary>
	/// Indicates the feature named "CD Audio External Play" by the MMC-3 specification. Devices that support this feature can play CD audio
	/// data and channel it directly to an external output.
	/// </summary>
	FeatureCDAudioAnalogPlay = 0x0103, // obsolete

	/// <summary>
	/// Indicates the feature named "Microcode Upgrade" by the MMC-3 specification. Devices that support this feature can upgrade their
	/// internal microcode by means of a published interface.
	/// </summary>
	FeatureMicrocodeUpgrade = 0x0104,

	/// <summary>
	/// Indicates the feature named "Time-Out" by the MMC-3 specification. Devices that have this feature must respond to commands within a
	/// set time period. When these devices cannot complete commands in the allotted time, they complete the commands with an error.
	/// </summary>
	FeatureTimeout = 0x0105,

	/// <summary>
	/// Indicates the feature named "DVD-CSS" by the MMC-3 specification. Devices that support this feature can perform DVD Content
	/// Scrambling System (DVD-CSS) authentication and key management.
	/// </summary>
	FeatureDvdCSS = 0x0106,

	/// <summary>
	/// Indicates the feature named "Real Time Streaming" by the MMC-3 specification. Devices that support this feature allow the initiator
	/// to specify the performance level of the device within certain limits allowed by the device. These devices must also indicate to the
	/// initiator whether they support stream playback operations.
	/// </summary>
	FeatureRealTimeStreaming = 0x0107,

	/// <summary>
	/// Indicates the feature named "Device Serial Number" by the MMC-3 specification. Devices that support this feature can furnish the
	/// initiator with a serial number that uniquely identifies the device.
	/// </summary>
	...
	/// <summary>
	/// Indicates the feature named "Disc Control Blocks" by the MMC-3 specification. Devices that support this feature can read or write
	/// Disc Control Blocks.
	/// </summary>
	FeatureDiscControlBlocks = 0x010A,

	/// <summary>
	/// Indicates the feature named "DVD CPRM" by the MMC-3 specification. Devices that support this feature can perform DVD Content
	/// Protection for Recordable Media (CPRM) authentication and key management.
	/// </summary>
...
{
	/// <summary>Does not indicate a valid profile.</summary>
	ProfileInvalid = 0x0000,

	/// <summary>
	/// Indicates the profile named "Nonremovable disk" by the SCSI-3 Multimedia (MMC-3) specification. This profile is used with devices
	/// that manage rewritable media and are capable of changing behavior.
	/// </summary>
	ProfileNonRemovableDisk = 0x0001,

	/// <summary>
	/// Indicates the profile named "Removable disk" by the MMC-3 specification. This profile is used with devices that manage rewritable,
	/// removable media.
	/// </summary>
	ProfileRemovableDisk = 0x0002,

	/// <summary>
	/// Indicates the profile named "MO Erasable" by the MMC-3 specification. This profile is used with devices that manage magneto-optical
	/// media and that have a sector-erase capability.
	/// </summary>
	ProfileMOErasable = 0x0003,

	/// <summary>
	/// Indicates the profile named "MO Write Once" by the MMC-3 specification. This profile is used with devices that manage magneto-optical
	/// write-once media.
	/// </summary>
	ProfileMOWriteOnce = 0x0004,

	/// <summary>
	/// Indicates the profile named "AS-MO" by the MMC-3 specification. This profile is used with devices that implement Advance Storage
	/// technology and manage magneto-optical media.
	/// </summary>
	ProfileAS_MO = 0x0005,
	...
	/// <summary>
	/// Indicates the profile named "DVD-R" by the MMC-3 specification. This profile is used with devices that manage write-once DVD media
	/// and operate in sequential recording mode.
	/// </summary>
	ProfileDvdRecordable = 0x0011,

	/// <summary>
	/// Indicates the profile named "DVD-RAM or DVD+RW" by the MMC-3 specification. This profile is used with devices that manage rewritable
	/// DVD media.
	/// </summary>
	ProfileDvdRam = 0x0012,

	/// <summary>
	/// Indicates the profile named "DVD-RW Restricted Overwrite" by the MMC-3 specification. This profile is used with devices that manage
	/// rerecordable DVD media and operate in packet-writing mode.
	/// </summary>
	ProfileDvdRewritable = 0x0013, // restricted overwrite

	/// <summary>
	/// Indicates the profile named "DVD-RW Sequential Recording" by the MMC-3 specification. This profile is used with devices that
	/// implement a series of features associated with sequential recording, such as the features "Incremental Streaming Writable" and
	/// "Real-Time Streaming". For a full list of the features supported with this profile, see the MMC-3 specification.
	/// </summary>
	...
	/// <summary>
	/// Indicates the profile named "DVD+RW" by the MMC-3 specification. This profile is used with devices that implement a series of
	/// features required to manage DVD media that is both readable and writable. For a full list of the features supported with this
	/// profile, see the MMC-3 specification.
	/// </summary>
	...
	/// <summary>
	/// Indicates the profile named "DDCD-ROM" by the MMC-3 specification. This profile is used with devices that can read "DDCD specific
	/// structure." For a full list of the features supported with this profile, see the MMC-3 specification.
	/// </summary>
	ProfileDDCdrom = 0x0020, // obsolete

	/// <summary>
	/// Indicates the profile named "DDCD-R" by the MMC-3 specification. This profile is used with devices that can read "DDCD-R specific
	/// structure." For a full list of the features supported with this profile, see the MMC-3 specification.
	/// </summary>
	ProfileDDCdRecordable = 0x0021, // obsolete

	/// <summary>
	/// Indicates the profile named "DDCD-RW" by the MMC-3 specification. This profile is used with devices that can read "DDCD-RW specific
	/// structure." For a full list of the features supported with this profile, see the MMC-3 specification.
	/// </summary>
	...
	/// <summary>Indicates that the device does not conform to any profile.</summary>
...
public struct DISC_INFORMATION
{
	...
	private byte bits1;
	public byte DiscStatus { readonly get => BitHelper.GetBits(bits1, 0, 2); set => BitHelper.SetBits(ref bits1, 0, 2, value); }
	public byte LastSessionStatus { readonly get => BitHelper.GetBits(bits1, 2, 2); set => BitHelper.SetBits(ref bits1, 2, 2, value); }
	public bool Erasable { readonly get => BitHelper.GetBit(bits1, 4); set => BitHelper.SetBit(ref bits1, 4, value); }
	...
	private byte bits2;
	public byte MrwStatus { readonly get => BitHelper.GetBits(bits2, 0, 2); set => BitHelper.SetBits(ref bits2, 0, 2, value); }
	public bool MrwDirtyBit { readonly get => BitHelper.GetBit(bits2, 2); set => BitHelper.SetBit(ref bits2, 2, value); }
	public bool URU { readonly get => BitHelper.GetBit(bits2, 5); set => BitHelper.SetBit(ref bits2, 5, value); }
	public bool DBC_V { readonly get => BitHelper.GetBit(bits2, 6); set => BitHelper.SetBit(ref bits2, 6, value); }
	public bool DID_V { readonly get => BitHelper.GetBit(bits2, 7); set => BitHelper.SetBit(ref bits2, 7, value); }
...
/// <remarks>
/// This structure holds data for the feature named "Profile List" by the MMC-3 specification. This feature provides a list of all profiles
/// supported by the device.
/// </remarks>
// https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddmmc/ns-ntddmmc-_feature_data_profile_list typedef struct
// _FEATURE_DATA_PROFILE_LIST { FEATURE_HEADER Header; FEATURE_DATA_PROFILE_LIST_EX Profiles[0]; } FEATURE_DATA_PROFILE_LIST, *PFEATURE_DATA_PROFILE_LIST;
...
{
	/// <summary>
	/// Contains a header that indicates how many profiles are reported in the profile list descriptor. The FEATURE_HEADER structure is used
	/// to describe both feature and profile list descriptors. When FEATURE_HEADER is used with a profile list descriptor the
	/// <c>FeatureCode</c> member of FEATURE_HEADER must be set to zero, the <c>Current</c> member must be set to 1, the <c>Version</c>
	/// member must be set to zero, and the <c>Persistent</c> member must be set to 1. The <c>Persistent</c> member is set to 1, because all
	/// devices that are compliant with the SCSI Multimedia - 4 (MMC-4) standard must support reporting of the profile list. The
	/// <c>AdditionalLength</c> member must be set to ((number of profile descriptors) * 4). See the MMC-3 specification For more information
	/// about the values assigned to these members.
	/// </summary>
...
/// </summary>
// https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddmmc/ns-ntddmmc-_feature_data_profile_list_ex typedef struct
// _FEATURE_DATA_PROFILE_LIST_EX { UCHAR ProfileNumber[2]; UCHAR Current : 1; UCHAR Reserved1 : 7; UCHAR Reserved2; }
...
public struct FEATURE_DATA_PROFILE_LIST_EX
{
	...
	/// <summary>
	/// Contains the profile number. This number must be one of the values defined by the FEATURE_PROFILE_TYPE enumeration.
	/// <c>ProfileNumber</c>[0] must contain the most significant byte of the profile number. <c>ProfileNumber</c>[1] must contain the least
	/// significant byte.
	/// </summary>
	public FEATURE_PROFILE_TYPE ProfileNumber { readonly get => (FEATURE_PROFILE_TYPE)BinaryPrimitives.ReverseEndianness(profileNumber); set => profileNumber = BinaryPrimitives.ReverseEndianness((ushort)value); }

	private byte flags;

	/// <summary>
	/// Indicates, when set to 1, that this feature is currently active and the feature data is valid. When set to zero, this bit indicates
	/// that the feature is not currently active and that the feature data might not be valid.
	/// </summary>
	public bool Current { readonly get => BitHelper.GetBit(flags, 0); set => BitHelper.SetBit(ref flags, 0, value); }
...
/// <summary>
/// The FEATURE_HEADER structure is used in conjunction with the IOCTL_CDROM_GET_CONFIGURATION request to report header information for both
/// feature and profile descriptors.
/// </summary>
// https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddmmc/ns-ntddmmc-_feature_header typedef struct _FEATURE_HEADER { UCHAR
// FeatureCode[2]; UCHAR Current : 1; UCHAR Persistent : 1; UCHAR Version : 4; UCHAR Reserved0 : 2; UCHAR AdditionalLength; } FEATURE_HEADER, *PFEATURE_HEADER;
...
public struct FEATURE_HEADER
{
	...
	/// </summary>
	public FEATURE_NUMBER FeatureCode { readonly get => (FEATURE_NUMBER)BinaryPrimitives.ReverseEndianness(featureCode); set => featureCode = BinaryPrimitives.ReverseEndianness((ushort)value); }

	private byte flags;

	/// <summary>
	/// Indicates, when set to 1, that this feature is currently active and the data reported for the feature is valid. When set to zero,
	/// this bit indicates that the feature is not currently active and that the data reported for the feature might not be valid.
	/// </summary>
	public bool Current { readonly get => BitHelper.GetBit(flags, 0); set => BitHelper.SetBit(ref flags, 0, value); }
	...
	/// </summary>
	public bool Persistent { readonly get => BitHelper.GetBit(flags, 1); set => BitHelper.SetBit(ref flags, 1, value); }

	/// <summary>Must be set to zero unless otherwise specified within the description for a particular feature.</summary>
	public byte Version { readonly get => BitHelper.GetBits(flags, 2, 4); set => BitHelper.SetBits(ref flags, 2, 4, value); }
...
// https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddmmc/ns-ntddmmc-_get_configuration_header typedef struct
// _GET_CONFIGURATION_HEADER { UCHAR DataLength[4]; UCHAR Reserved[2]; UCHAR CurrentProfile[2]; UCHAR Data[0]; } GET_CONFIGURATION_HEADER, *PGET_CONFIGURATION_HEADER;
...
public struct GET_CONFIGURATION_HEADER
{
	...
	/// <summary>
	/// Indicates the amount of data, in bytes, that is being returned in the buffer area pointed to by the <c>Data</c> member. If the data
	/// length is greater than 65,530 bytes, multiple GET CONFIGURATION commands will be required for the Initiator to read all configuration
	/// data. The bytes in this array are arranged in big-endian order. <c>DataLength</c>[0] has the most significant byte, and
	/// <c>DataLength</c>[3] has the least significant byte.
	/// </summary>
	public uint DataLength { readonly get => BinaryPrimitives.ReverseEndianness(dataLength); set => dataLength = BinaryPrimitives.ReverseEndianness(value); }
	...
	/// <summary>
	/// Contains an enumerator value of type FEATURE_PROFILE_TYPE that indicates the device's current profile. The bytes in this array are
	/// arranged in big-endian order. <c>CurrentProfile</c>[0] has the most significant byte, and <c>CurrentProfile</c>[3] has the least
	/// significant byte.
	/// </summary>
	public FEATURE_PROFILE_TYPE CurrentProfile { readonly get => (FEATURE_PROFILE_TYPE)BinaryPrimitives.ReverseEndianness(currentProfile); set => currentProfile = BinaryPrimitives.ReverseEndianness((ushort)value); }
}
...
public struct OPC_TABLE_ENTRY
{
	...
	public ushort Speed { readonly get => (ushort)BitHelper.GetBits(bytes, 0, 16); set => BitHelper.SetBits(ref bytes, 0, 16, value); }
	public ulong OPCValue { readonly get => (ushort)BitHelper.GetBits(bytes, 16, 48); set => BitHelper.SetBits(ref bytes, 16, 48, value); }
}
...
// https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddcdvd/ns-ntddcdvd-_dvd_layer_descriptor typedef struct
// _DVD_LAYER_DESCRIPTOR { UCHAR BookVersion : 4; UCHAR BookType : 4; UCHAR MinimumRate : 4; UCHAR DiskSize : 4; UCHAR LayerType : 4; UCHAR
// TrackPath : 1; UCHAR NumberOfLayers : 2; UCHAR Reserved1 : 1; UCHAR TrackDensity : 4; UCHAR LinearDensity : 4; ULONG StartingDataSector;
// ULONG EndDataSector; ULONG EndLayerZeroSector; UCHAR Reserved5 : 7; UCHAR BCAFlag : 1; } DVD_LAYER_DESCRIPTOR, *PDVD_LAYER_DESCRIPTOR;
...
public struct DVD_LAYER_DESCRIPTOR
{
	...
	public byte BookVersion { readonly get => (byte)BitHelper.GetBits(bits, 0, 4); set => BitHelper.SetBits(ref bits, 0, 4, value); }
	...
	/// </list>
	/// </summary>
	public byte BookType { readonly get => (byte)BitHelper.GetBits(bits, 4, 4); set => BitHelper.SetBits(ref bits, 4, 4, value); }
	...
	/// </list>
	/// </summary>
	public byte MinimumRate { readonly get => (byte)BitHelper.GetBits(bits, 8, 4); set => BitHelper.SetBits(ref bits, 8, 4, value); }
	...
	public byte DiskSize { readonly get => (byte)BitHelper.GetBits(bits, 12, 4); set => BitHelper.SetBits(ref bits, 12, 4, value); }
	...
	/// </list>
	/// </summary>
	public byte LayerType { readonly get => (byte)BitHelper.GetBits(bits, 16, 4); set => BitHelper.SetBits(ref bits, 16, 4, value); }

	/// <summary>
	/// Specifies the direction of the layers when more than one layer is used. If the <c>TrackPath</c> member is zero, this media uses a
	/// parallel track path (PTP). With PTP, each layer is independent and has its own lead-in and lead-out areas. If <c>TrackPath</c> is 1,
	/// the media uses opposite track path (OTP). With opposite track path, the two layers are united, and there is only one lead-in and
	/// lead-out area. For further details, see the SCSI Multimedia Commands - 3 (MMC-3) specification.
	/// </summary>
	public bool TrackPath { readonly get => BitHelper.GetBit(bits, 20); set => BitHelper.SetBit(ref bits, 20, value); }

	/// <summary>
	/// Specifies the number of layers present on the side of the media being read. A value of zero indicates that the media has one layer. A
	/// value of 1 indicates that the media has two layers.
	/// </summary>
	public byte NumberOfLayers { readonly get => (byte)BitHelper.GetBits(bits, 21, 2); set => BitHelper.SetBits(ref bits, 21, 2, value); }
	...
	/// </list>
	/// </summary>
	public byte TrackDensity { readonly get => (byte)BitHelper.GetBits(bits, 24, 4); set => BitHelper.SetBits(ref bits, 24, 4, value); }

	/// <summary>
	/// <para>
	/// Indicates the minimum/maximum pit length used for this layer in units of micrometers per bit. This member can have one of the
	/// following values:
	/// </para>
	/// <list type="table">
	/// <listheader>
	...
	/// </list>
	/// </summary>
	public byte LinearDensity { readonly get => (byte)BitHelper.GetBits(bits, 28, 4); set => BitHelper.SetBits(ref bits, 28, 4, value); }
	...
	/// <summary>
	/// Specifies the last sector of the user data in layer zero. If this media does not use the opposite track path method and contains
	/// multiple layers, this value is set to zero.
	/// </summary>
	...
	/// </summary>
	public bool BCAFlag { readonly get => BitHelper.GetBit(flags, 7); set => BitHelper.SetBit(ref flags, 7, value); }
}

/// <summary>The FEATURE_DATA_CORE structure holds data for the Core feature descriptor.</summary>
/// <remarks>
/// Indicates the feature named "Core" by the MMC-3 specification. This feature encompasses the basic functionality which is mandatory for
/// all devices that support the MMC-3 standard. See the MMC-3 specification for a description of the capabilities included in the Core feature.
/// </remarks>
// https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddmmc/ns-ntddmmc-_feature_data_core typedef struct _FEATURE_DATA_CORE {
// FEATURE_HEADER Header; UCHAR PhysicalInterface[4]; UCHAR DeviceBusyEvent : 1; UCHAR INQUIRY2 : 1; UCHAR Reserved1 : 6; UCHAR Reserved2[3];
...
public struct FEATURE_DATA_CORE
{
	...
	/// <summary>
	/// Must be set to the current communication path between initiator and device, as defined in the SCSI Multimedia - 4 (MMC-4)
	/// specification. The bytes of this array are arranged in big-endian order. <c>PhysicalInterface</c>[0] contains the most significant
	/// byte, and <c>PhysicalInterface</c>[3] contains the least significant byte.
	/// </summary>
	[MarshalAs(UnmanagedType.ByValArray, SizeConst = 4)]
	public byte[] PhysicalInterface;

	private byte bits;

	/// <summary/>
	public bool DeviceBusyEvent { readonly get => BitHelper.GetBit(bits, 0); set => BitHelper.SetBit(ref bits, 0, value); }

	/// <summary/>
	public bool INQUIRY2 { readonly get => BitHelper.GetBit(bits, 1); set => BitHelper.SetBit(ref bits, 1, value); }
}
...
// 0x010A - FeatureDiscControlBlocks an integral multiple of the EX structures are returned for page 010A
...
// NOTE: All FEATURE_* structures may be extended. use of these structures requires verification that the FeatureHeader.AdditionLength field
// contains AT LEAST enough data to cover the data fields being accessed. This is due to the design, which allows extending the size of the
// various structures, which will result in these structures sizes being changed over time. A 0-element array is however not declared in the
// variable size structures. Such array is declared on some structures to preserve legacy, yet it is deprecated. To access variable size
// structures, as they are always at the end of the fixed size structure, use a sizeof of the declared fixed size structure as an offset.
// *** Programmers beware! ***

// NOTE: This is based on MMC 3, extended to MMC 5 rev 3 Further revisions will maintain backward compatibility with the non-reserved fields
// listed here. If you need to access a new field, please typecast to FEATURE_DATA_RESERVED and access the appropriate bits there.

// IOCTL_CDROM_GET_CONFIGURATION returns a FEATURE_* struct, which always starts with a FEATURE_HEADER structure.
